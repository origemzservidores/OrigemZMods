modded class InGameMenu extends UIScriptedMenu
{
	...
	bool m_enabletutorial = false;

	override Widget Init()
	{
		layoutRoot = GetGame().GetWorkspace().CreateWidgets("OrigemZUI/gui/layouts/cui.day_z_ingamemenu.layout");
		...
		m_GameOverScreenImage.LoadImageFile(0, GameOverScreen.GetRandomGameOverScreen());
		m_GameOverScreenImage.SetAlpha(0);


		m_tutorialbtn = Widget.Cast(layoutRoot.FindAnyWidget("tutorialbtn"));
