...
class UIColor
{
	...
	static int ORZRed()               { return ARGB(255, 255, 0, 0); };
}
...
class colorScheme 
{
	...
	// Shader Colors 
	static int TopShader()       	  { return UIColor.Black(); };
	static int BottomShader()    	  { return UIColor.CUI2SubtleRed(); };

	// Global UI Elements 
	static int ButtonHover()     	  { return BrandColor(); };	
	static int LogOutTimer()     	  { return UIColor.White(); };	
	...
	static int LoadingMsg()       	  { return UIColor.White(); };
	...
	static int WelcomeBack()     	  { return UIColor.Yellow(); };
