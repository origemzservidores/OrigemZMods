...
{	
	protected const string m_DataPath = OrigemZHudVars.origemz_hints_json;
	protected string m_RootPath       = "OrigemZUI/Gui/layouts/new_ui/hints/cui.in_game_hints.layout";
    ...
    override protected void SetHintImage()
	{
		...
		{	
            ...
            private DayZGame dzgame = DayZGame.Cast(GetGame());
            if (!dzgame)
            {
                m_UiHintImage.LoadImageFile(0, OrigemZHudVars.origemz_tela_logo);
                return;
            }
...
modded class UiHintPanelLoading extends UiHintPanel
{
    ...
    {
        m_RootPath = "OrigemZUI/gui/layouts/new_ui/hints/cui.in_game_hints_load.layout";
