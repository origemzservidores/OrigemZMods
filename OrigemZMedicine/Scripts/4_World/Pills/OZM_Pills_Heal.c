...
{	
	...
	{
		if (consumer.GetModifiersManager().IsModifierActive(new_OZM_eModifiers.MDF_HEAL_PILLS))
		{ 
			HealPillsMdfr mdf = consumer.GetModifiersManager().GetModifier(new_OZM_eModifiers.MDF_HEAL_PILLS);
			if (mdf)
			{
		...
		{
			consumer.GetModifiersManager().ActivateModifier(new_OZM_eModifiers.MDF_HEAL_PILLS);
		}
	}
};

class HealPillsMdfr: ModifierBase
{
	...
	override void Init()
	{
		...
		m_IsPersistent = true;
		m_ID 					= new_OZM_eModifiers.MDF_HEAL_PILLS;
	...
	override void OnTick(PlayerBase player, float deltaT)
	{
		...
		if (curren_heath < max_health)
			player.AddHealth("GlobalHealth", "Health", (max_health * 0.002) * ACUMULATIVE_HEALS); // Healls 5% of max health in 10 seconds
	}
};